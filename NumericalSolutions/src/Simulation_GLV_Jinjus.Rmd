...
write.csv(df_species_N_7_Jinjus_03, file = "c_001_deterministic/df_species_N_7.csv")
```
...
lbl_x <- "Species"
lbl_y <- "Species"
ggplot(mat_df_2, aes(Var2, Var1, fill = value)) +
  geom_tile(color = "white") +  # Plot tiles
  geom_text(aes(label = round(value, 1)), color = "black") +  # Add value text
  scale_fill_gradient(low = "white", high = "blue") +
  labs(title = lbl_title, x = lbl_x, y = lbl_y) +
...
```{r}
df_species %>% 
  dplyr::mutate(AbsAbun2 = 10^AbsAbun)
```
